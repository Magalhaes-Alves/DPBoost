...
def process_fold(dataset_name, epsilon, fold, train_idx, test_idx, X, y, plot_dir, feature_names):
    ...
    lgb_params = {
        ...
        "lambda_l2": 0.1, "bagging_freq": 1, "bagging_fraction": 1, "max_bin": 64,
        "total_budget": epsilon, "boost_method": "DPBoost_2level", 
        "high_level_boost_round": 1, "inner_boost_round": 1,
    ...
    # createScatterPlotReconstruction(X_original, X_reconstruido_ts, f"{plot_dir}/TS_PCA_fold_{fold}", method='pca')
    # createScatterPlotReconstruction(X_original, X_reconstruido_ts, f"{plot_dir}/TS_TSNE_fold_{fold}", method='tsne')
    ...
    # attacker_reboot.phase_3(eta=lgb_params['learning_rate'], reg_lambda=lgb_params['lambda_l2'])
    ...
    #     createScatterPlotReconstruction(X_original, amostras_reconstruidas, f"{plot_dir}/REBOOT_PCA_fold_{fold}", method='pca')
    #     createScatterPlotReconstruction(X_original, amostras_reconstruidas, f"{plot_dir}/REBOOT_TSNE_fold_{fold}", method='tsne')

    # Retorna os resultados deste fold específico
    return {
        "Dataset": dataset_name,"f1_score":f1 ,"Epsilon": epsilon, "Fold": fold,
    }
...
if __name__ == "__main__":
    ...
    df_results.to_csv("results/DPBOOST_ATTACK_CONSOLIDATED_BREAST_f1.csv", index=False)
